...
class HabitRepository(private val habitDao: HabitDao, private val executor: ExecutorService) {
    ...
    fun getHabits(filter: HabitSortType): LiveData<PagedList<Habit>> {
        val factory = when (filter) {
            HabitSortType.START_TIME -> habitDao.getHabits(SortUtils.getSorteredQuery(filter))
            HabitSortType.MINUTES_FOCUS -> habitDao.getHabits(SortUtils.getSorteredQuery(filter))
            HabitSortType.TITLE_NAME -> habitDao.getHabits(SortUtils.getSorteredQuery(filter))
        }
        val config =
