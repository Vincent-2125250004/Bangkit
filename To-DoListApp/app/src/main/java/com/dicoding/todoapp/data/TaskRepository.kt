...
class TaskRepository(private val tasksDao: TaskDao) {
    ...
    fun getTasks(filter: TasksFilterType): LiveData<PagedList<Task>> {
        val factory = when (filter) {
            TasksFilterType.ALL_TASKS -> tasksDao.getTasks(FilterUtils.getFilteredQuery(filter))
            TasksFilterType.ACTIVE_TASKS -> tasksDao.getTasks(FilterUtils.getFilteredQuery(filter))
            TasksFilterType.COMPLETED_TASKS -> tasksDao.getTasks(FilterUtils.getFilteredQuery(filter))
        }

        val config = PagedList.Config.Builder().setPageSize(PAGE_SIZE).setEnablePlaceholders(
            PLACEHOLDERS
        ).build()
