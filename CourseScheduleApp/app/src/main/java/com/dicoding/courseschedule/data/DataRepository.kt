...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getAllCourse(sortType: SortType): LiveData<PagedList<Course>> {
        val factory = when (sortType) {
            SortType.COURSE_NAME -> dao.getAll(QueryUtil.sortedQuery(SortType.COURSE_NAME))
            SortType.TIME -> dao.getAll(QueryUtil.sortedQuery(SortType.TIME))
            SortType.LECTURER -> dao.getAll(QueryUtil.sortedQuery(SortType.LECTURER))
        }
